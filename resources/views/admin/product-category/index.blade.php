            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header pb-0">
                        <a href="{{ route('product-category.create') }}" class="btn btn-primary">Add New Item</a>
                        <h6>Table Product Category</h6>
                    </div>
                            ...
                            <table class="table align-items-center mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 text-center">Category Name</th>
                                        <th class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <!-- Loop through product-categories data -->
                                    @foreach($categories as $index => $category)
                                    <tr>
                                        <td class="text-center">{{ $index + 1 }}</td>
                                        <td class="text-center">{{ $category->category_name}}</td>
                                        <td class="align-middle text-center text-sm">
                                            <a href="{{ route('product-category.edit', $category->id) }}" class="badge badge-sm bg-gradient-success">Edit</a>
                                            ...
                                            <form id="frmDelete{{ $category->id }}" action="{{ route('product-category.destroy', $category->id) }}" method="POST" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </td>
