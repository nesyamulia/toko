...
<div class="card mb-4">
    ...
    <form action="{{ route('product-review.update', $review->id) }}" method="post" id="ProductReviewForm">
        @csrf
        @method('PUT')
            ...
            <div class="border-contrast p-1 rounded"> 
                <select class="form-select text-white border-0" id="customer_id" name="customer_id">
                    ...
                    @foreach($customers as $customer)
                        <option value="{{ $customer->id }}" class="text-dark" {{ $customer->id == $review->customer_id ? 'selected' : '' }}>{{ $customer->name }}</option>
                    @endforeach
                </select>
            </div>
            @error('customer_id')
            ...
            <div class="border-contrast p-1 rounded"> 
                <select class="form-select text-white border-0" id="product_id" name="product_id">
                    ...
                    @foreach($products as $product)
                        <option value="{{ $product->id }}" class="text-dark" {{ $product->id == $review->product_id ? 'selected' : '' }}>{{ $product->product_name }}</option>
                    @endforeach
                </select>
            </div>
            @error('product_id')
        ...
        <div class="mb-3 ms-3 me-3">
            ...
            <div class="border-contrast p-1 rounded"> 
                <input type="number" class="form-control text-white border-0" id="rating" name="rating" value="{{ $review->rating }}" class="text-dark">
            </div>
            @error('rating')
                <div class="error-message">{{ $message }}</div>
            @enderror
        ...
        <div class="mb-3 ms-3 me-3">
            ...
            <div class="border-contrast p-1 rounded"> 
                <textarea class="form-control text-white border-0" id="comment" name="comment">{{ $review->comment }}</textarea>
            </div>
            @error('comment')
                <div class="error-message">{{ $message }}</div>
            @enderror
        ...
        <div class="row ms-3 me-3 justify-content-end">
            ...
            <div class="col-3">
                <button type="submit" id="update" class="btn bg-gradient-primary w-100">Update</button>
            </div>
        </div>
    </form>
</div>
...
<script>
    const btnSave = document.getElementById("update"); // Mengganti ID dengan ID tombol update
    ...
    function simpan() {
        ...
        if (errorMessages.length > 0) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: 'Do you want to update this product review?', // Mengganti teks sesuai dengan konteks
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Yes, update it!' // Mengganti teks sesuai dengan konteks
            }).then((result) => {
